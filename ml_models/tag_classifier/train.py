...
VAL_SIZE        = 0.25      # 25 % of 80 % = 20 % overall  ->  60/20/20 split
...
def load_and_clean(csv_path: str):
    ...
    # 5 — Build combined text feature (strip any HTML tags)
    def _clean(s):
        s = re.sub(r'<[^>]+>', ' ', str(s))   # strip HTML tags
        s = re.sub(r'\s+', ' ', s).strip()     # collapse whitespace
        return s
    df["text"] = df["title"].fillna("").apply(_clean) + ". " + df["problem_description"].fillna("").apply(_clean)
...
def _word_tfidf():
    """
    Word-level TF-IDF with trigrams.

    - ngram_range=(1,3) : captures 'binary search tree', 'depth first search'
    - max_features=50000: generous vocabulary for multi-label discrimination
...
def _char_tfidf():
    """Character-level ngrams capture code patterns like O(n), dp[i], TreeNode."""
    return TfidfVectorizer(
        max_features=20000,
        ngram_range=(3, 5),
        sublinear_tf=True,
...
def _combined_features():
    """FeatureUnion: word ngrams + character ngrams for richer representation."""
    return FeatureUnion([
        ("word", _word_tfidf()),
        ("char", _char_tfidf()),
    ])
...
def get_model_definitions():
    ...
    Three feature strategies tested:
      1. Word trigrams (1-3) -- captures multi-word concepts
      2. Word + character ngrams -- adds character-level code patterns
      3. Word trigrams with SGD -- fast alternative

    class_weight='balanced' compensates for label imbalance.
    """
    return {
        "LR balanced (word 1-3)": (
            Pipeline([
                ("tfidf", _word_tfidf()),
            ...
            {"clf__estimator__C": [10.0, 50.0, 100.0, 200.0]},
        ),
        "LR balanced (word+char)": (
            Pipeline([
        ...
        "SGD balanced (word 1-3)": (
            ...
            {"clf__estimator__alpha": [1e-5, 5e-5, 1e-4, 5e-4]},
        ),
        "Chain LR (word+char)": (
            Pipeline([
                ("features", _combined_features()),
            ...
            {"clf__estimator__C": [50.0, 100.0, 200.0]},
        ),
    }
...
def optimize_thresholds(pipeline, X_val, Y_val, mlb):
    ...
    # ── Step A: Find optimal GLOBAL threshold ─────────────────────────────────
    # A single threshold is more robust than per-label thresholds on small
    # validation sets because it aggregates evidence across all labels.
    best_global = 0.5
    best_global_f1 = 0.0
    for t in np.arange(0.15, 0.70, 0.01):
        Y_tmp = (proba >= t).astype(int)
        for row in range(len(Y_tmp)):
            if Y_tmp[row].sum() == 0:
                Y_tmp[row, proba[row].argmax()] = 1
        f1 = f1_score(Y_val, Y_tmp, average="micro", zero_division=0)
        if f1 > best_global_f1:
            best_global_f1 = f1
            best_global = t
    print(f"  Global optimal threshold: {best_global:.2f}  (F1-micro={best_global_f1:.4f})")

    # ── Step B: Per-label thresholds with shrinkage toward global ──────────────
    # Regularise per-label thresholds to prevent overfitting on the small
    # validation set.  The final threshold is a blend:
    #   final = shrinkage * per_label_optimal + (1 - shrinkage) * global
    SHRINKAGE = 0.5
    thresholds = {}

    for i, label in enumerate(mlb.classes_):
        best_thresh = best_global
        best_f1     = 0.0
        for t in np.arange(0.10, 0.80, 0.025):
            ...
            if f1 > best_f1:
                best_f1     = f1
                best_thresh = t
        # Regularise: blend per-label optimum with global optimum
        blended = SHRINKAGE * best_thresh + (1 - SHRINKAGE) * best_global
        thresholds[label] = round(float(blended), 3)
        print(f"    {label:<30}  optimal={best_thresh:.3f}  blended={blended:.3f}  F1={best_f1:.4f}")

    # ── Step C: Evaluate with regularised thresholds ──────────────────────────
    ...
    print(f"\n  Regularised threshold results (validation set):")
