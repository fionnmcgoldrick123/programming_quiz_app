    ...
    try:
        ...
        for i, test_case_str in enumerate(request.test_cases):
            try:
                ...
                result = await execute_code_with_test(request.code, request.language, input_data, expected)
                ...
                test_results.append({"test_number": i + 1, "passed": False, "error": f"Invalid test case format: {str(e)}"})
                all_passed = False
...
async def execute_code_with_test(code: str, language: str, input_data: dict, expected):
    ...
    try:
        if lang == "python":
            ...
            return {"passed": False, "error": f"Language '{language}' not supported for testing"}
    except Exception as e:
